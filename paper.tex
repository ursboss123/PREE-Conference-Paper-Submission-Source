...
\usepackage{url}
\usepackage[font=scriptsize,labelfont=bf,skip=1pt]{caption}
...
\author{
\IEEEauthorblockN{Rahul Rajeevkumar Urs and Abdelrahman E. Eltoukhy}
\IEEEauthorblockA{
Khalifa University\\
Abu Dhabi, United Arab Emirates\\
rahul.urs@ku.ac.ae; abdelrahman.eltoukhy@ku.ac.ae}
}

\maketitle

\begin{abstract}
This paper presents a reliability-constrained sizing framework for a photovoltaic (PV)--electrolyser--fuel cell--hydrogen storage--grid microgrid serving a 24/7 data centre, with an existing N+1 diesel genset retained as residual backup. Hourly grid-connected dispatch is run to hydrogen-inventory steady state and coupled to a 5000-event Monte Carlo outage assessment. For a 5~MW critical-IT data centre in Abu Dhabi, the lowest-cost sampled-reliable design uses 22~MW PV, a 10~MW electrolyser, a 5~MW fuel cell and a 17{,}102~kg (57-hour) H$_2$ tank; it has zero unmet energy over those sampled events and an LCOE of 0.1272~\$~kWh\textsuperscript{-1}. Relative to a grid-plus-diesel baseline serving the same design-basis outage energy, it cuts annual CO\textsubscript{2} by 44.5\% at a 52.8\% cost increase, equivalent to \$233.6~tCO\textsubscript{2}\textsuperscript{-1}. The central result is structural: unlike a diurnally cycling, power-rate-limited battery, the H$_2$ tank fills opportunistically over the year, so sampled outage survival rises with autonomy until the design-basis outage draw is covered.
\end{abstract}
...
Existing work covers renewable/hydrogen sizing in power networks \cite{kayacik2024} and data-centre fuel-cell economics \cite{kurtz2023}, but not the joint problem of Monte Carlo outage survival, retained-diesel retrofit practice, and autonomy sweeps that reveal whether reliability saturates before all sampled outage energy is covered.
...
For each sizing, $N=5000$ outage events $(t_0^{(i)}, d^{(i)})$ are drawn from a shared design-basis mixture rather than fitted site records: 60\% transient, $U(0.25,2)$~h; 30\% medium, $U(2,12)$~h; and 10\% extended, $U(12,96)$~h. Each event is simulated in islanded mode from the steady-state hydrogen inventory at $t_0^{(i)}$, yielding unmet energy $u^{(i)}$. The empirical Outage Survival Probability is
\begin{equation}
P_{\mathrm{survive}} = \frac{1}{N}\sum_{i=1}^{N} \mathbb{1}\!\left[u^{(i)} \le \varepsilon\right],
\label{eq:psurvive}
\end{equation}
i.e.\ the fraction of sampled events fully served by PV+H\textsubscript{2} alone ($\varepsilon=10^{-9}$~MWh). Thus $P_{\mathrm{survive}}=1.000$ means no failures in this 5000-event design envelope, not zero risk outside it. A complementary energy-coverage fraction,
\begin{equation}
...
Islanded PV surplus is curtailed rather than routed to the electrolyser, so the tank cannot recharge during the event. This conservative reserve policy makes the selected point reliability-tight: the tank ends at 0.1\%, and each extra hour of full-power fuel-cell reserve would add about 300~kg of usable H$_2$ by Eq.~(\ref{eq:h2mass}) if margin beyond the sampled events is required.
...
The labelled frontier also clarifies the grey design cloud. Several high-capacity combinations approach the selected reliability level at higher LCOE, because added PV or autonomy is useful only when it changes the worst residual outage draw. Low-cost points are attractive for energy procurement but remain backup-incomplete. Thus the planning question is not whether hydrogen is cheaper than grid supply, but what premium removes sampled diesel dispatch while retaining the genset as contingency.
...
Annualised cost rises from \$4.838M~yr$^{-1}$ to \$7.392M~yr$^{-1}$, while CO\textsubscript{2} falls from 24.6 to 13.7~kt~yr$^{-1}$, giving a 52.8\% cost premium, 44.5\% CO\textsubscript{2} reduction and \$233.6~tCO\textsubscript{2}$^{-1}$ abatement. Proposed-system cost comprises PV \$1.663M, electrolyser \$1.492M, fuel cell \$0.839M, hydrogen tank \$1.121M, net grid exchange \$1.819M, and diesel fixed cost \$0.458M~yr$^{-1}$. Since $\mathrm{EUE}_{\mathrm{yr}}=0$ within the sampled design-basis events, the genset burns no fuel and $f_E=100\%$ in this comparison.

A local $\pm20\%$ perturbation of annualised PV-H$_2$ component costs, without re-optimising size, moves proposed LCOE from 0.108 to 0.146~\$~kWh$^{-1}$ and abatement cost from about \$140 to \$328~tCO\textsubscript{2}$^{-1}$. Reliability is unchanged because physical size is unchanged, but component-specific sensitivities could shift the least-cost point.

Relative to the companion battery result, hydrogen removes sampled diesel energy rather than merely reducing it, but at a larger premium. Fig.~\ref{fig:comparison} is therefore a resilience/emissions trade-off, not a least-cost expansion result: existing diesel remains available outside the Monte Carlo sample, while sampled design-basis events are served by PV and stored hydrogen.

Across the figures, the resource profile defines the PV-surplus window and cooling penalty; the outage dispatch shows residual night-time tank duty; the heat map locates the 57-h reliability threshold; the Pareto front prices the move to zero sampled unmet energy; and the baseline comparison confirms a decarbonising resilience design rather than a least-cost energy-procurement case.

\section{Conclusion}

This study presented a reliability-constrained sizing framework for a PV-electrolyser-fuel-cell-H$_2$-grid data-centre microgrid with retained diesel backstop. For a 5~MW Abu Dhabi case, the lowest-cost sampled-reliable design uses 22~MW PV and a 57-hour tank, achieving $P_{\mathrm{survive}}=1.000$ over 5000 design-basis events, $\mathrm{CFE}=0.443$ and $\mathrm{LCOE}=0.1272$~\$~kWh$^{-1}$. It cuts CO\textsubscript{2} by 44.5\% at a 52.8\% cost premium under base assumptions. Future work should test hybrid battery-hydrogen sizing, measured outage/weather sequences, multiple climates, component-specific sensitivities and electrolyser operation during extended outages.
